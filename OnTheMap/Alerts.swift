...
class UAlerts {
    ...
    class func show(type: AlertType, at: UIViewController, withTextFrom: AsMessageShowable) {
        let alertTitle = withTextFrom.toMessageTitle()
        let alertMessage = withTextFrom.toMessageBody()
        self.show(type, at: at, withText: alertMessage, andTitle: alertTitle)
    }
